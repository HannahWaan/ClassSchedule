function renderGroups() {
    ...
    el.innerHTML = Store.groups.map(function(g) {
        ...
        return '<div class="group-card"><div style="display:flex;justify-content:space-between;align-items:flex-start"><div><h4>👥 '+g.name+'</h4><p>📚 '+(g.program||'—')+' · 💰 '+formatMoney(g.rate)+'/'+g.rate_minutes+'p (nhóm)</p>'+(g.note?'<p>📝 '+g.note+'</p>':'')+'<div class="group-members">'+members.map(function(m){return'<span class="group-member-tag">'+m.name+'</span>';}).join('')+'</div></div><button class="btn btn-danger btn-sm" onclick="deleteGroup(\''+g.id+'\')">🗑</button></div></div>';
    }).join('');
}
async function saveGroup(e) {
    ...
    var checks = document.querySelectorAll('#g-members-list input:checked');
    for(var i=0;i<checks.length;i++){ var sid=checks[i].value; await db.from('students').update({group_id:ng.id}).eq('id',sid); var stu=Store.students.find(function(s){return s.id===sid;}); if(stu)stu.group_id=ng.id; }
    document.getElementById('group-form').reset(); closeGroupModal(); renderGroups(); populateGroupDropdown(); syncUI('✅ Synced');
}
