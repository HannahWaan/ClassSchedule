function renderStudents() {
    ...
    el.innerHTML = Store.students.map(function(s) {
        return '<div class="stu-card"><div class="stu-info"><h4>' + s.name + '</h4><p>📚 ' + (s.program || '—') + ' · 🎯 ' + s.level + '</p>' + (s.review ? '<p>💬 ' + s.review + '</p>' : '') + (s.note ? '<p>📝 ' + s.note + '</p>' : '') + '</div><button class="btn btn-danger btn-sm" onclick="deleteStudent(\'' + s.id + '\')">🗑</button></div>';
    }).join('');
}
...
function populateDropdown() {
    ...
    sel.innerHTML = '<option value="">-- Chọn --</option>' + Store.students.map(function(s) { return '<option value="' + s.id + '">' + s.name + '</option>'; }).join('');
}
