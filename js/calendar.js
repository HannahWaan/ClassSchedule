...
function renderWeek(root) {
    ...
    for (var hr = 6; hr <= 22; hr++) {
        ...
        for (var d2 = 0; d2 < 7; d2++) {
            ...
            cs.forEach(function(s) { h += '<div class="cal-ev" title="' + s.lesson + '">' + (s.student_name || s.lesson) + '</div>'; });
            h += '</div>';
        }
    }
    h += '</div>';
...
function renderMonth(root) {
    ...
    for (var day = 1; day <= last.getDate(); day++) {
        ...
        h += '<div class="cal-mday' + (today ? ' today' : '') + '"><div class="dn">' + day + '</div>';
        ss.slice(0, 2).forEach(function(s) { h += '<div class="cal-ev">' + s.start_time.slice(0, 5) + ' ' + (s.student_name || s.lesson) + '</div>'; });
        if (ss.length > 2) h += '<div style="font-size:.65rem;color:var(--text3)">+' + (ss.length - 2) + '</div>';
        h += '</div>';
    }
    h += '</div>';
    root.innerHTML = h;
...
function renderDay(root) {
    ...
    if (!ss.length) h += '<p class="muted">Không có buổi dạy nào hôm nay.</p>';
    else ss.forEach(function(s) {
        h += '<div class="day-item"><strong>' + s.lesson + '</strong><span>' + (s.student_name || '') + ' · ' + s.start_time.slice(0, 5) + '–' + s.end_time.slice(0, 5) + ' · ' + formatMoney(s.fee) + '</span>' + (s.note ? '<br><em style="font-size:.82rem;color:var(--text3)">' + s.note + '</em>' : '') + '<br><button class="btn btn-danger btn-sm" style="margin-top:10px" onclick="deleteSession(\'' + s.id + '\')">🗑 Xóa</button></div>';
    });
    h += '</div>';
    root.innerHTML = h;
}
...
function updateCalNav() {
    ...
    if (calView === 'week') {
        ...
        el.textContent = 'Tháng ' + (calDate.getMonth() + 1) + ' / ' + calDate.getFullYear();
    } else {
        el.textContent = fmtDate(fmtISO(calDate));
    }
}
