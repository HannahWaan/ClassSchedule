...
function updateDashboard() {
  ...
  if(el('today-list')){
    ...
    else el('today-list').innerHTML = todaySessions.map(function(s){
      var d=new Date(s.date); var time=d.toLocaleTimeString('vi-VN',{hour:'2-digit',minute:'2-digit'});
      var icon=s.status==='Done'?'✅':'🕐';
      return '<div class="s-item"><div class="s-item-info"><strong>'+icon+' '+(s.name||s.student)+'</strong><span>'+time+' · '+(s.duration||0)+'p · '+formatVND(s.fee||0)+'</span></div></div>';
    }).join('');
  }
  ...
  if(el('week-list')){
    ...
    else el('week-list').innerHTML = weekList.map(function(s){
      var d=new Date(s.date); var day=d.toLocaleDateString('vi-VN',{weekday:'short',day:'numeric',month:'numeric'});
      var time=d.toLocaleTimeString('vi-VN',{hour:'2-digit',minute:'2-digit'}); var icon=s.status==='Done'?'✅':'🕐';
      return '<div class="s-item"><div class="s-item-info"><strong>'+icon+' '+(s.name||s.student)+'</strong><span>'+day+' '+time+' · '+formatVND(s.fee||0)+'</span></div></div>';
    }).join('');
  }
}
...
function buildMonthBar() {
  ...
  if (!bar) return;
  // Build tabs from May 2026 to current month
  var START_YEAR = 2026, START_MONTH = 4; // 0-indexed: May = 4
  var now = new Date();
  var months = [{label:'Tất cả', value:null}];
  var y = START_YEAR, m = START_MONTH;
  while (y < now.getFullYear() || (y === now.getFullYear() && m <= now.getMonth())) {
    var val = y + '-' + String(m+1).padStart(2,'0');
    months.push({label:'T'+(m+1)+'/'+y, value:val});
    m++; if(m>11){m=0;y++;}
  }
  bar.innerHTML = months.map(function(item) {
    var active = (item.value === currentStatsMonth) ? ' active' : '';
    var onclick = item.value === null ? 'setStatsMonth(null)' : "setStatsMonth('"+item.value+"')";
    return '<button class="ptab'+active+'" onclick="'+onclick+'">'+item.label+'</button>';
  }).join('');
}

function setStatsMonth(m) { currentStatsMonth = m; buildMonthBar(); updateStats(); }
...
function updateStats() {
  ...
  students.forEach(function(st){
    ...
    var earned,expected;
    if(st.feeType==='per-session'||st.feeType==='free-session'){earned=stDone.length*(st.fee||0);expected=stSessions.length*(st.fee||0);}
    else{var dm=new Set(),am=new Set();stDone.forEach(function(s){var d=new Date(s.date);dm.add(d.getFullYear()+'-'+d.getMonth())});stSessions.forEach(function(s){var d=new Date(s.date);am.add(d.getFullYear()+'-'+d.getMonth())});earned=dm.size*(st.fee||0);expected=am.size*(st.fee||0);}
    totalEarned+=earned; totalExpected+=expected;
  });
  ...
  document.querySelectorAll('.stab').forEach(function(btn){
    btn.onclick=function(){document.querySelectorAll('.stab').forEach(function(b){b.classList.remove('active')});this.classList.add('active');currentStatsFilter=this.dataset.filter;renderStatsDetail(filtered,students);};
  });
}

function renderStatsDetail(filtered, students) {
  var detail=document.getElementById('stats-detail'); if(!detail)return;
  var list=students;
  ...
  detail.innerHTML=list.map(function(st){
    ...
    return '<div class="stats-student-row"><div><span class="stats-student-name">'+st.name+'</span> '+statusTag+'</div><span class="stats-student-info">'+stDone.length+'/'+stSessions.length+' buổi · '+formatVND(earned)+'</span></div>';
  }).join('');
}
