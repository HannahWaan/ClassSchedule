function renderWelcome() {
    ...
    if (!up.length) el.innerHTML = '<p class="muted">Chưa có buổi dạy nào.</p>';
        ...
        var icon = s.session_type === 'group' ? '👥' : '👤';
        return '<div class="s-item"><div class="s-item-info"><strong>' + icon + ' ' + s.lesson + '</strong><span>' + (s.student_name || s.group_name || '') + ' · ' + fmtDate(s.date) + ' · ' + s.start_time.slice(0,5) + '–' + s.end_time.slice(0,5) + '</span></div></div>';
    }).join('');
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.querySelectorAll('.vtab[data-view]').forEach(function(b) {
        b.addEventListener('click', function() {
            ...
            b.classList.add('active'); calView = b.dataset.view; renderCalendar();
        });
    });
    // Stats periods
    ...
    // Auto fee recalc
    ['f-start','f-end','f-student','f-group'].forEach(function(id) {
        var el = document.getElementById(id);
        if (el) el.addEventListener('change', recalcFee);
    });
    ['q-start','q-end','q-student','q-group'].forEach(function(id) {
        var el = document.getElementById(id);
        if (el) el.addEventListener('change', recalcQuickFee);
    });

    // Hamburger
    document.getElementById('hamburger').addEventListener('click', function() { document.getElementById('sidebar').classList.toggle('open'); });
    document.querySelector('.main').addEventListener('click', function() { document.getElementById('sidebar').classList.remove('open'); });
});
